...
max.publication <- filter(pulitzer, Pulitzer.Prize.Change==max(Pulitzer.Prize.Change)) %>% select(Newspaper)
...
publications.decrease <- filter(pulitzer, Pulitzer.Prize.Winners.and.Finalists..1990.2003 > 5)
